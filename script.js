...
const savedColors = ['FF498D', 'FC944C', getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor()];
const presets = {
  1: {
    colors: ["FF0000", "FF7F00", "FFFF00", "00FF00", "0000FF", "4B0082", "9400D3"],
  },
  2: {
    colors: ["FF498D", "FC944C"],
  },
  3: {
...
function updateOutputText(event) {
  ...
  if (format.outputPrefix) {
    ...
    if (nickName.value) {
      ...
      if (!nickName.value.match(letters)) nickName.value = '';
    }
  }

  let newNick = nickName.value
  if (!newNick) {
    newNick = '¡Escribe algo!'
  }
